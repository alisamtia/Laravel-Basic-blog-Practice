...
<div class="border p-5 rounded-3xl">
    ...
    <a href="{{ route('index',['category'=>$post->category->id]) }}"><p class="font-sans mt-2 font-md ml-1 text-blue-500 text-md">{{ ucwords($post->category->name) }}</p></a>
    <a href="{{route('post.show',$post->id)}}"><h3 class="{{$type=='featured'?'text-3xl':'lg:text-2xl text-xl'}} mt-2 font-sans m-none font-semibold">{{ $post->title }}</h3></a>
    <div class="{{$type=='normal'?'lg:flex':'flex'}} mt-3 gap-2">
        <a href="{{ route('index',['category'=>$post->author->id]) }}">
